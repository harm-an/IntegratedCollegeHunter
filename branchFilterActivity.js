...
const states = require('./branches.json')
var selectedbranches = new Set()
var selectedBranchesArray = []

export default class BranchFilterActivity extends Component{
        ...
        if(states[i].includes(this.state.text) ){
            sugg.push(
              <TouchableWithoutFeedback onPress={this.addLocations.bind(this, states[i])} key={i}>
                <View style={{backgroundColor: colors.statusBarDark, width: w-16, height: 100, marginTop: 0, borderRadius: 0, alignItems:'center',flexDirection: 'row' }} key={i}>
                  <Text style={{marginLeft: 16, color: 'white', marginRight: 50}}>{states[i]}</Text>
                  <Image style={{tintColor: (selectedbranches.has(states[i]))?colors.accentBright:colors.primary, height: 20, width: 20, right: 16, position: 'absolute'}} source={(selectedbranches.has(states[i]))?require('./icons/tick.png'):require('./icons/add.png')}/>
                </View>
              </TouchableWithoutFeedback>
            )
        }
          ...
          return (
            <View style={styles.container}>
              ...
              <View style={[styles.navBar, {justifyContent: 'center'}]}>
                    ...
                    <Text style={{fontFamily: 'Montserrat-Medium', left: 56, color: colors.primary, fontSize: 18}}>Course</Text>
                </View> 
              ...
              {(true )?<View style={{backgroundColor: colors.statusBarDark, position: 'absolute', height:(Platform.OS == 'android')?h-56-56-56+40:h-56-56-56+40+10,width: w, top: (Platform.OS == 'android')?56+56:56+56+10, elevation: 0}}>
                <ScrollView>
